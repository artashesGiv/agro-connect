...
- `src/services/posts/postsRepository.ts` — the feed: one select with all joins (author profile, crop, type, stage, status, media, plus raw `post_reactions` rows and an `answers(count)` aggregate) instead of N follow-up queries. Filtering on an embedded table needs `!inner`, otherwise PostgREST nulls the embedded object instead of dropping the parent row — hence `feedSelect(innerPostType)`. Lives in `services` because three consumers reuse it: `features/profile/hooks/useUserPosts.ts` (`getFeed({ authorId })`, no paging), `features/home/hooks/useFeed.ts` (all posts, infinite scroll), and `getPost(id)` for `PostDetail`. Infinite scroll uses a **compound keyset cursor**: `order('created_at desc, id desc')` + `before: { createdAt, id }` → `.or('created_at.lt.X,and(created_at.eq.X,id.lt.Y)')`, because `created_at` is not unique. Both feed hooks sign `post-media` URLs, fold reaction rows into per-type summaries, and expose `syncItem(id)` — a one-post `getPost(id)` re-map the screens fire from `useFocusEffect` after returning from `PostDetail`/`EditPost` (comment count, reactions, edited fields) without reloading the list. Profile also does a full `reload()` when it arrives with `route.params.refresh` (set by the create wizard so a new post shows). `updatePost` / `updatePostWithMedia` (partial PATCH + `post_media` diff, incl. Storage file removal) power the edit wizard.
